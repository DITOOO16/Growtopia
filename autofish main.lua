...
function autoFish()
	sendMessage('`9Do "!startfish" to start autofishing! - Type: MAIN')
	stopFish()
	    ...
	    if v[0]=="OnConsoleMessage" then
			...
			if pmsg__ and pmsg__:sub(1,1)=='!' then
				...
				elseif pmsg__[1]=='setfish' then
					...
					if not pmsg__[2] then
						water_y=loc.tile.y+1
						water_x=loc.tile.x+({[false]=1,[true]=-1})[loc.facing_left]
						sendMessage('`6Set Position to '..water_x..', '..water_y..' - Facing: '..({[false]='Right',[true]='Left'})[loc.facing_left])

					else
						if pmsg__[2]=='left' then
							water_x,water_y=loc.tile.x-1,loc.tile.y+1
